	...
	bool prefiltered)
{
	...
	float3 p_3D = {orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2]};
	...
	conic_opacity[idx] = { conic.x, conic.y, conic.z, opacity_final };
	tiles_touched[idx] = (rect_max.y - rect_min.y) * (rect_max.x - rect_min.x);
}
...
renderCUDA(
	...
	float* __restrict__ out_depth_gaussians)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (T > 0.5 && median_depth == 0.)
				median_depth = collected_depth[j];
	...
	if (inside)
	{
		...
		out_depth_gaussians[pix_id] = median_depth;
	}
}
